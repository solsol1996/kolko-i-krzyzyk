...
// Inicjalizujemy aktualnego gracza
let currentPlayer = "X";

// Inicjalizujemy pustą planszę
let gameBoard = ["", "", "", "", "", "", "", "", ""];
// Tablica do przechowywania zwycięskiej kombinacji
...
function createBoard() {
    for (let i = 0; i < 9; i++) {
        ...
        cell.dataset.index = i; // Każda komórka ma swój unikalny index
        cell.addEventListener('click', handleCellClick); // Dodajemy obsługę kliknięcia
...
function handleCellClick(event) {
    ...
    event.target.textContent = currentPlayer; // Ustawiamy symbol gracza w klikniętej komórce
    gameBoard[event.target.dataset.index] = currentPlayer; // Aktualizujemy stan gry
    ...
    if (checkWin()) {
        ...
        drawWinningLine(); // Rysujemy linię zwycięstwa
    } else {
...
function checkWin() {
    ...
    for (const condition of winConditions) {
        ...
        if (gameBoard[a] && gameBoard[a] === gameBoard[b] && gameBoard[a] === gameBoard[c]) {
            winningCombo = condition; // Zapisujemy zwycięską kombinację
            return true; // Zwracamy true, jeśli jest zwycięzca
        }
    }
...
function resetGame() {
    gameBoard = ["", "", "", "", "", "", "", "", ""]; // Resetujemy planszę
    currentPlayer = "X"; // Resetujemy gracza
    const cells = document.querySelectorAll(".cell");
    cells.forEach((cell) => {
        cell.textContent = ""; // Usuwamy tekst w komórkach
        cell.addEventListener("click", handleCellClick); // Dodajemy możliwość kliknięcia ponownie
    });
    ...
    // Resetujemy komunikat
    document.getElementById("message").textContent = `${gracz_1}`;
}
...
document.getElementById('notifyButton').addEventListener('click', function() {
    ...
    // Jeśli powiadomienie jest ukryte, pokazujemy je
    if (notification.style.display === 'none' || notification.style.display === '') {
        ...
        // Jeśli powiadomienie jest już widoczne, ukrywamy je
        notification.style.display = 'none';
    }
});
